...
func (tm *TraderManager) LoadTradersFromDatabase(database *config.Database) error {
	...
	for _, traderCfg := range allTraders {
		...
		if exchangeCfg == nil {
			exchangeCfg = buildSimulationExchangeIfNeeded(traderCfg)
			if exchangeCfg == nil {
...
func buildSimulationExchangeIfNeeded(traderCfg *config.TraderRecord) *config.ExchangeConfig {
	switch traderCfg.ExchangeID {
	case "paper", "simulation":
