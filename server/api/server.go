...
func (s *Server) initUserDefaultConfigs(userID string) error {
	if err := s.database.CreateExchange(userID, "paper", "Paper Trading", "simulation", true, "", "", false, "", "", "", ""); err != nil {
		log.Printf("⚠️ 初始化用户 %s 的模拟交易所失败: %v", userID, err)
		return err
	}

	log.Printf("用户 %s 注册完成，已启用模拟交易所 (paper)", userID)
	return nil
}
