...
func (d *Database) initDefaultData() error {
	...
	exchanges := []struct {
		...
		{"binance", "Binance Futures", "binance", false},
		{"hyperliquid", "Hyperliquid", "hyperliquid", false},
		{"aster", "Aster DEX", "aster", false},
		{"paper", "Paper Trading", "simulation", true},
	}
	...
	for _, exchange := range exchanges {
		_, err := d.db.Exec(`
			...
			VALUES (?, 'default', ?, ?, ?)
		`, exchange.id, exchange.name, exchange.typ, boolToInt(exchange.enabled))
		if err != nil {
